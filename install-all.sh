apt update && apt full-upgrade -y && apt install -y snapd nano wget curl git htop sshfs nfs-common qemu-guest-agent && systemctl start qemu-guest-agent && systemctl enable qemu-guest-agent \
