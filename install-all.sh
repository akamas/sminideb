apt update && apt full-upgrade -y && apt install -y snapd nano wget curl git htop sshfs qemu-guest-agent make && systemctl start qemu-guest-agent && systemctl enable qemu-guest-agent \
